...
- /reference/tivaware/pwmsignals/
- /reference/libraries/otp3001/
...
| 3 | MIC Functionality | ADC reads, level calculation and vertical bar with color display | 30 |
...
**Screen 1 — Ambient Light (LUX)** is **not required for lab submission**, as some development boards may have issues with the light sensor (OPT3001).
</Aside>
...
* Draw a **vertical bar** that grows and changes in color with sound intensity.
...
- Microphone (MIC) functionality — 13 points total:
  - ADC configured and reads microphone channel AIN8 / PE5 (12 pts).
  - Level computed (average or RMS) and displayed numerically if required (7 pts).
  - Vertical bar visual responds to sound intensity and updates smoothly (7 pts).
  - Non-blocking sampling and UI responsiveness (4 pts).
