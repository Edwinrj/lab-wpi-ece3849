...
{
    float level = Mic_Level(); // 0.0  1.0 normalized volume

    // Step 1 � Clear drawing area
    tRectangle bg = {0, 21, 127, 127};
    GrContextForegroundSet(&gContext, ClrBlack);
    GrRectFill(&gContext, &bg);

    // Step 2 � Draw header (title + arrows)
    DrawHeader("Microphone", btnLeft_isPressed(), btnRight_isPressed());

    // Step 3 � Define meter area
    const int barX1 = 55, barX2 = 73, barY1 = 25, barY2 = 120;
    const int barHeight = barY2 - barY1;

    // Step 4 � Clamp level to [0,1]
    if (level < 0) level = 0;
    if (level > 1) level = 1;

    // Step 5 � Compute filled portion
    int filled = (int)(level * barHeight);
    int yStart = barY2 - filled;

    // Step 6 � Draw color segments (green  yellow  red)
    ...
    // Step 8 � Send final buffer to LCD
    GrFlush(&gContext);
}
```
