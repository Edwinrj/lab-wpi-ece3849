...
3. `ButtonTask` wakes up immediately, processes the buttons, and blocks again.

#### Setup checklist

- Include `semphr.h`
- Create the semaphore before enabling the GPIO interrupt
- Configure falling-edge GPIO interrupts for the physical pins used by **S1** and **S2**
- If your project maps `S1` and `S2` to the LaunchPad user switches, those pins will be **PJ0** and **PJ1**
- If your project maps `S1` and `S2` to BoosterPack buttons, use the corresponding GPIO ports and pins from your board mapping instead
- In the ISR, clear the interrupt flag, give the semaphore, and yield if needed
- In `ButtonTask`, block on `xSemaphoreTake()` and call `btnS1.tick()` / `btnS2.tick()` after wake-up
...
In this example, `BTN_PORT_BASE`, `BTN_PIN_MASK`, and `BTN_INT_NUM` stand for the GPIO port, pin mask, and interrupt number that match your actual `S1` and `S2` wiring. If both buttons are on the same GPIO port, one mask can cover both pins. If they are on different ports, configure each port separately and point both interrupts to the same ISR.
...
<CardGrid>
  <LinkCard
    ...
    href="/reference/libraries/joystick/"
    icon="right-arrow"
  />
</CardGrid>
