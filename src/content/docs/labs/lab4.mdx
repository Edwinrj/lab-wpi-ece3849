...
6. **Measure the improvement**
   - Use your timing measurement code from Part 3
   - Document the new LCD execution time
...
4. **Reliability Assessment**:
   ...
   - Artifacts observed: Yes / No
   - Description of any issues: _____________
   - Final recommended frequency: _____ MHz
...
A **counting semaphore** tracks a count of available resources:
- **Post** (signal): Increments count, wakes waiting tasks
- **Pend** (wait): Decrements count, blocks if zero
- **Count**: Number of available resources/pending signals

Perfect for buffering inputs where order matters!

### Implementation Strategy

1. **Interrupt-driven joystick reading** for instant response
2. **Counting semaphore** to signal new inputs
3. **Queue or buffer** to store directional commands
4. **Snake task** processes buffered commands
...
### Direction Queue Setup

```c
// In main(), after creating other synchronization objects:
...
static void vInputTask(void *pvParameters)
{
    ...
    Direction lastDirection = gameState.currentDirection;
    
    for (;;) {
        // Hardware button + joystick polling (keep existing code)
        btnPause.tick();
        btnReset.tick();
        gJoystick.tick();

        // Process buttons (existing code)
        ...
        // Joystick direction mapping (existing code)
        Direction newDirection = lastDirection;  // Keep current by default
        ...
        // NEW: Only queue valid direction changes (not reversals)
        if (newDirection != lastDirection && 
            newDirection != GetOppositeDirection(gameState.currentDirection)) {
            
            // Try to add to queue (non-blocking)
            if (xQueueSend(xQueueDirections, &newDirection, 0) == pdPASS) {
                xSemaphoreGive(xSemJoystickInput);  // Signal new input
                lastDirection = newDirection;
            }
            // If queue full, drop the input (prevents lag)
        }
...
static void vSnakeTask(void *pvParameters)
{
    ...
    for(;;) {
        // Process ALL queued direction changes
