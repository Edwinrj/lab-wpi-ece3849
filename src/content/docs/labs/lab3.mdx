...
- **Snake movement:** Represent the snake as an array of positions updated every tick.
- **Fruit placement:** Randomly place fruit on empty grid cells.
- **Collision detection:** The game ends when the snake hits itself or a wall.
- **Growth:** When the snake eats a fruit, increase its length.

> Students must design the logic; no implementation is provided.

---

## Tips

- Do not draw or access LCD without acquiring the mutex.
- Use consistent update rates (e.g., `vTaskDelayUntil()` for predictable timing).
- Keep code modular and readable — do not mix hardware and game logic.

---

## Deliverables

- Modular source code (`buzzer`, `display`, `game`, `tasks`, `app_objects`, `main`).
- Proper mutex protection around LCD access.
- Working Snake game with moving fruit, collision detection, and score tracking.
- A short **report or README** explaining mutex usage.

---

## Challenge Ideas

- Add a game-over blink animation or tone sequence.
- Gradually increase snake speed as score increases.
- Add a short sound cue when the fruit is eaten.

---

> **End of Lab 3 — Snake (FreeRTOS)**  
> Continue following the modular structure introduced in Labs 1 and 2. Emphasize concurrency, mutex use, and task organization over full gameplay polish.
