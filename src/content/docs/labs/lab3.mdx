...
**❌ Mistake 1: Forgetting to give back a mutex**
```c
...
**✅ Solution: Always use structured patterns**
```c
...
**❌ Mistake 2: Calling FreeRTOS functions from ISR without "FromISR" version**
```c
...
**❌ Mistake 3: Long delays or blocking while holding mutex**
```c
...
**✅ Solution: Release mutex before delays**
```c
...
**❌ Mistake 4: Using wrong wait time for queue operations**
```c
...
| Buzzer | 1 | **Lowest** - Audio feedback is non-critical |
...
- **Buzzer task** (priority 1) plays sounds when nothing else needs CPU
...
<details>
<summary>💡 Hint 1: Queue Size Reasoning (Click if unsure about queue size)</summary>
...
<details>
<summary>💡 Hint 2: Task Priority Logic (Click if stuck on priority decision)</summary>

Ask yourself:
- Should audio interruptions stop game logic? (No → Lower priority than Snake)
- Should missing button presses stop audio? (No → Same or lower priority than Input)  
...
<details>
<summary>💡 Hint 3: Queue Operations (Click if queue communication isn't working)</summary>
...
<details>
<summary>💡 Hint 4: PWM Integration (Click if no sound comes out)</summary>
...
<details>
<summary>💡 Hint 1: Data Structure Setup (Click if stuck on variable declarations)</summary>
...
<details>
<summary>💡 Hint 2: Eating Detection Logic (Click if stuck on position comparison)</summary>
...
<details>
<summary>💡 Hint 3: Snake Growth Algorithm (Click if snake growth acts weird)</summary>
...
<details>
<summary>💡 Hint 4: Reset Integration (Click if score doesn't reset properly)</summary>
...
<details>
<summary>🔊 Hint 1: Sound Integration Location (Click if sound doesn't play when eating)</summary>
...
<details>
<summary>🔊 Hint 2: Frequency Experimentation (Click for sound design guidance)</summary>
...
<details>
<summary>🔊 Hint 3: Sound Timing Validation (Click if multiple sounds overlap)</summary>
...
<details>
<summary>💡 Hint A1: Opposite Direction Function (Click for function implementation)</summary>
...
<details>
<summary>💡 Hint A2: Input Validation Logic (Click for joystick integration)</summary>
...
<details>
<summary>💡 Hint B1: Collision Detection Function (Click for implementation)</summary>
...
<details>
<summary>💡 Hint B2: Collision Integration (Click for vSnakeTask integration)</summary>
...
<details>
<summary>💡 Hint B3: Testing Collision Scenarios (Click for test cases)</summary>

**Manual test scenarios:**
```c
// Test case 1: Length 4 snake moving into itself
// Snake: Head(5,5), Body(4,5), Body(3,5), Tail(2,5)
// Move LEFT → Head goes to (4,5) → Collision with Body(4,5) ✓
...
// UP→RIGHT→DOWN→LEFT quickly → Should hit body
```
...
<details>
<summary>💡 Hint C1: Game State Variables (Click if stuck on state management)</summary>
...
<details>
<summary>💡 Hint C2: Game Over Display (Click for display integration)</summary>
...
<details>
<summary>💡 Hint C3: Reset Logic Update (Click for complete reset function)</summary>
...
1. **Stack overflow**
   - Each task has limited stack space defined at creation
   - Check if any task is using too much local variables or deep recursion
   
   ```c
   // Increase stack size if tasks crash
   xTaskCreate(vSnakeTask, "Snake", 512, NULL, 2, NULL);
   //                                ^^^
   //                                Try 1024 if crashing
   ```

2. **Mutex never released**
   - Use the pattern: Take → Do work → Give
   - Add debug prints to verify mutex is released
...
3. **Queue handle not initialized**
   - Always check if queue/mutex creation succeeded
   
   ```c
   xBuzzerQueue = xQueueCreate(10, sizeof(BuzzerEvent));
   if (xBuzzerQueue == NULL) {
       printf("ERROR: Failed to create queue!\n");
       while(1);  // Halt system to make error obvious
   }
   ```

### Symptom: Display corruption or flickering

**Likely cause:** Multiple tasks drawing without mutex protection

**Solution:**
```c
// EVERY function that touches LCD must use mutex
void DrawAnything() {
    xSemaphoreTake(xMutexLCD, portMAX_DELAY);
    // All GrContext... calls here
    xSemaphoreGive(xMutexLCD);
}
```

### Symptom: Input lag or unresponsive controls

**Possible causes:**

1. **Task blocking while holding mutex**
   - Release mutex BEFORE delays
   
2. **Wrong task priority**
   - Input task should be highest priority
   
3. **Queue full, blocking sends**
   - Use timeout of 0 for non-critical queues
   
   ```c
   // Don't block on audio events
   xQueueSend(xBuzzerQueue, &event, 0);  // Returns immediately if full
   ```

### Useful Debug Techniques

**1. Add task status monitoring:**
```c
// Temporarily add to a low-priority task
...
```c
// Add counters to see if tasks are running
static uint32_t inputCounter = 0;
static void vInputTask(void* pvParameters) {
    for(;;) {
        inputCounter++;  // Should increment rapidly
        // ... rest of task
    }
}
```
...
- If high-priority task waits for mutex held by low-priority task
- FreeRTOS handles this with priority inheritance
- But be aware of the concept for understanding behavior

<Aside type="tip">
Use `printf()` debugging sparingly in high-frequency tasks (Input, Render) as it can slow down execution significantly. Use counters and LEDs for frequent debugging.
</Aside>
...
4. **What's the difference between queue depth and item size?**
   ...
   - Total queue memory = depth × item size (10 × 8 = 80 bytes)
   </details>
