...
This minimal example creates a **single task** that increments a counter on the LCD once per second. If you see the number counting up, FreeRTOS is working.
...
// A single task: count up on the LCD every second
void CounterTask(void *pvParameters) {
    ...
    for (;;) {
        ...
        GrStringDrawCentered(&sContext, "FreeRTOS OK", -1, 64, 40, false);

        // Draw counter
        snprintf(buf, sizeof(buf), "%lu", count);
        GrContextForegroundSet(&sContext, ClrWhite);
        GrStringDrawCentered(&sContext, buf, -1, 64, 70, false);
...
int main(void) {
    ...
    xTaskCreate(CounterTask, "Counter", 512, NULL, 1, NULL);
...
Compile and flash the project to your EK-TM4C1294XL board. You should see **"FreeRTOS OK"** on the LCD with a number counting up every second. This confirms:
- The project compiles and links with FreeRTOS
- The scheduler starts successfully
- `vTaskDelay()` works (the counter increments at a steady 1-second rate)
