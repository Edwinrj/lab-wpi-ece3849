...
## Pattern: Timer Wakes a Worker Task

The most common and correct pattern is to split work into two pieces:

```text
software timer callback
    -> do the small time-critical step (read sensor, store sample)
    -> when work is accumulated, signal the worker task

Worker task
    -> waits for the signal
    -> does the heavier processing (math, display update, logging)
```

This keeps callbacks short and leaves computation in tasks where blocking and longer runtimes are acceptable.
...
Software timers are best when you need a **small, regular trigger**. The timer callback creates a consistent rhythm; a dedicated worker task does the heavier processing when the callback signals that enough data or time has accumulated.
